...
void table_column_sort(table *t, int *cols, table_order *sort_orders, int num_cols)
{
	...
	for (sort_column = 0; sort_column < num_cols; sort_column++)
	{
		...
		else
		{
			...
			table_column *col_ptr = table_get_col_ptr(t, cols[sort_column - 1]);
			for (row = 0; row < num_rows; row++)
			{
                if (row)
                {
                    ...
                    {
                       equal = !col_ptr->compare(table_get(t, row, cols[prior_column]), table_get(t, row - 1, cols[prior_column]));
                       if (!equal)
                         break;
                    }
                    if (equal)
                ...
                //if (first != -1 && (last == row-1 || (last == row && row == num_rows-1)))
...
void table_merge_sort_merge_rows(table *t, table_row *sorted_rows, int col, int first, int middle, int last, table_order order)
{
	...
	for (i=0 ; i<last-first+1 ; i++)
	{
		...
		{
			if (n1 <= middle && (n2 > last || col_ptr->compare(table_get(t, n1, col), table_get(t, n2, col)) < 0))
			{
		...
		else if (order == TABLE_DESCENDING)
		{
			if (n1 <= middle && (n2 > last || col_ptr->compare(table_get(t, n1, col), table_get(t, n2, col)) > 0))
			{
				sorted_rows[i] = *table_get_row_ptr(t, n1);
